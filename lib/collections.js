...
const topics = (collection) => {
  ...
  // then the pages' front-matter will be used as the data source for the topic.
  const indexPages = [
  ...
  const usedTopicSet = Array.from(collection.getAll().reduce((tags, post) => {
 ...
 *
 * This is used for displaying special pages such as the blogroll, which is essentially
 * a special view on bookmark posts tagged with list/blogroll. Week in Review and my
 * 365-Day project also have special views for their output.
 *
