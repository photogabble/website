...
export const collectionStats = async (collection) => {
  ...
  const stats = await collection.reduce(async (carry, item) => {
    const stats = await carry;

    stats.totalItems++;
    if (stats.firstItem === null) stats.firstItem = item;

    // Need to await the templateContent getter as it depends on async Template methods, this should stop 11ty crashing
    // when reloading during development mode.
    const templateContent = await item.template.read();
