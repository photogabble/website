...
/**
 * Extracts a single element from a page content by its class name. For example,
 * to extract footnotes from a page content so it can be output elsewhere in the
 * template, you would use `extractElementByQuery('section.footnotes')`.
 *
 * @param {string|undefined} html
 * @param {string} query
 * @return {string|undefined}
 */
export const extractElementByQuery = (html, query) => {
  if (!html) return html;
  const root = parse(html);
  const element = root.querySelector(query);

  return (element) ? element.toString() : undefined;
}

/**
 * Filters the elements matching `query` from HTML.
 * @param {string|undefined}html
 * @param {string} query
 * @return {string|undefined}
 */
export const withoutElementsByQuery = (html, query) => {
    if (!html) return html;
    const root = parse(html);
    const elements = root.querySelectorAll(query);

    if (!elements) return html;
    elements.forEach(element => root.removeChild(element));
...
export const collectionStats = (collection) => {
  ...
  const stats = collection.reduce((stats, item) => {
    ...
    if (!templateContent) return stats;
...
export const registerFilters = (eleventyConfig) => {
  ...
  eleventyConfig.addFilter('extractElementByQuery', extractElementByQuery);
  eleventyConfig.addFilter('withoutElementsByQuery', withoutElementsByQuery);
  ...
  eleventyConfig.addAsyncFilter('collectionStats', collectionStats);
  ...
  // This is used by page-post.njk for displaying the series sidebar component
  ...
  eleventyConfig.addFilter('merge', (value, other) => {
