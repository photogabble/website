...
export const dateToFormat = (date, format) => {
  if (typeof date === 'object' && typeof date.toFormat === 'function') {
    return date.toFormat(String(format));
  }
...
export const specialTagMeta = (list) => {
  ...
  const permalink = meta.permalink ?? `/lists/${slugged}/`;
 ...
 * @return {string|false}
 */
export const specialTagValue = (tags, key) => {
 ...
 * @returns {boolean}
 */
export const includesTag = (tags, slug) => tags.find(tag => tag.toLowerCase() === slug.toLowerCase()) !== undefined;

/**
 * Excludes special tags denoted by `:` or `/`
 *
 * @param {Array<String>} tags
 * @returns {Array<String>}
 */
export const excludeSpecialTags = (tags) => tags.filter(tag => (tag.includes(':') && tag.includes('/')) === false);
...
export const groupBySpecialTag = (collections, tag, filter = []) => {
  ...
  return Object.keys(collections)
    ...
    .reduce((carry, collection) => {
      ...
      const data = metadata.specialTags[collection] ?? {};
